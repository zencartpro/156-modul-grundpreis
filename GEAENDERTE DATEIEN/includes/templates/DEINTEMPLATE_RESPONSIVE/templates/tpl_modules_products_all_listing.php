 ...
 * @version $Id: tpl_modules_products_all_listing for Grundpreis 2019-07-21 08:33:58Z webchills $
 */
?>
  ...
  if ($products_all_split->number_of_rows > 0) {
    ...
    while (!$products_all->EOF) {
      ...
      if (PRODUCT_ALL_LIST_MODEL != '0' and zen_get_show_product_switch($products_all->fields['products_id'], 'model')) {
        $display_products_model = '<b>' . TEXT_PRODUCTS_MODEL . '</b>' . $products_all->fields['products_model'] . str_repeat('<br />', substr(PRODUCT_ALL_LIST_MODEL, 3, 1));
      } else {
        $display_products_model = '';
      }

      if (PRODUCT_ALL_LIST_WEIGHT != '0' and zen_get_show_product_switch($products_all->fields['products_id'], 'weight')) {
        $display_products_weight = '<b>' . TEXT_PRODUCTS_WEIGHT . '</b>' . $products_all->fields['products_weight'] . TEXT_SHIPPING_WEIGHT . str_repeat('<br />', substr(PRODUCT_ALL_LIST_WEIGHT, 3, 1));
      } else {
      ...
      if (PRODUCT_ALL_LIST_QUANTITY != '0' and zen_get_show_product_switch($products_all->fields['products_id'], 'quantity')) {
        if ($products_all->fields['products_quantity'] <= 0) {
          $display_products_quantity = TEXT_OUT_OF_STOCK . str_repeat('<br />', substr(PRODUCT_ALL_LIST_QUANTITY, 3, 1));
        } else {
          $display_products_quantity = '<b>' . TEXT_PRODUCTS_QUANTITY . '</b>' . $products_all->fields['products_quantity'] . str_repeat('<br />', substr(PRODUCT_ALL_LIST_QUANTITY, 3, 1));
        }
      } else {
      ...
      if (PRODUCT_ALL_LIST_DATE_ADDED != '0' and zen_get_show_product_switch($products_all->fields['products_id'], 'date_added')) {
        $display_products_date_added = '<b>' . TEXT_DATE_ADDED . '</b> ' . zen_date_long($products_all->fields['products_date_added']) . str_repeat('<br />', substr(PRODUCT_ALL_LIST_DATE_ADDED, 3, 1));
      } else {
        $display_products_date_added = '';
      }

      if (PRODUCT_ALL_LIST_MANUFACTURER != '0' and zen_get_show_product_switch($products_all->fields['products_id'], 'manufacturer')) {
        $display_products_manufacturers_name = ($products_all->fields['manufacturers_name'] != '' ? '<b>' . TEXT_MANUFACTURER . '</b> ' . $products_all->fields['manufacturers_name'] . str_repeat('<br class="clearBoth" />', substr(PRODUCT_ALL_LIST_MANUFACTURER, 3, 1)) : '');
      } else {
      ...
      if ((PRODUCT_ALL_LIST_PRICE != '0' and zen_get_products_allow_add_to_cart($products_all->fields['products_id']) == 'Y') and zen_check_show_prices() == true) {
        ...
        if (zen_not_null($products_all->fields['products_base_unit_price']) && $products_all->fields['products_base_unit_price'] <> 0) {
		        $display_products_price .= TEXT_PRODUCTS_BASE_UNIT_PRICE . ' ' . $currencies->format(zen_round($products_all->fields['products_base_unit_price'],3)).'/'.$products_all->fields['products_base_unit'].'<br/>';
...
//            $how_many++;
        ...
        $the_button = $link;
        $products_link = '<a class="list-more" href="' . zen_href_link(zen_get_info_page($products_all->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_all->fields['master_categories_id']) . '&products_id=' . $products_all->fields['products_id']) . '">' . MORE_INFO_TEXT . '</a>';
