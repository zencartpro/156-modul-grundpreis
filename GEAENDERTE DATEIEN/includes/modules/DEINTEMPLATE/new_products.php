 ...
 * @version $Id: new_products.php for Grundpreis 2019-07-21 08:03:16Z webchills $
 */
...
if ( (($manufacturers_id > 0 && empty($_GET['filter_id'])) || !empty($_GET['music_genre_id']) || !empty($_GET['record_company_id'])) || empty($new_products_category_id) ) {
  $new_products_query = "SELECT DISTINCT p.products_id, p.products_image, p.products_tax_class_id, pd.products_name,
                                p.products_date_added, p.products_price, p.products_base_unit_price, p.products_base_unit, p.products_type, p.master_categories_id
...
// add for base_unit_price 2006.06.10 CCS-cart #joco 
    if (zen_not_null($new_products->fields['products_base_unit_price']) && $new_products->fields['products_base_unit_price']<>0 ) 
				$products_price .= '<br />'.TEXT_PRODUCTS_BASE_UNIT_PRICE . ' ' . $currencies->format(zen_round($new_products->fields['products_base_unit_price'],3)).'/'.$new_products->fields['products_base_unit'].'<br />';
// end add base price
